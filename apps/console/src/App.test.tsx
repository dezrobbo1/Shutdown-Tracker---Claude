...
describe("controlled export", () => {
  ...
  it("emits a line only for the fields an update actually carries", () => {
    const lines = previewLinesFor(update());

    expect(lines.map((line) => line.fieldName)).toEqual(["percent_complete", "actual_start"]);
    expect(lines.every((line) => line.sourceEntityType === "task_progress_update")).toBe(true);
    expect(lines.every((line) => line.sourceEntityId === "update-1")).toBe(true);
  });

  it("emits nothing for an update that changes no schedule field", () => {
    expect(previewLinesFor(update({ percentComplete: null, actualStart: null }))).toEqual([]);
  });
