...
import type {
  ExportBatchState,
  ExportPreviewDetail,
  ExportPreviewLineCreateRequest,
...
export function ExportZone({ session, client }: ZoneProps) {
  ...
  const createPreview = async () => {
    ...
    const lines = candidates.flatMap((update) => previewLinesFor(update));
    if (lines.length === 0) {
      return;
    }
...
/**
 * The export lines an approved update implies.
 *
 ...
 */
export function previewLinesFor(update: TaskProgressUpdateRecord): ExportPreviewLineCreateRequest[] {
  const lines: ExportPreviewLineCreateRequest[] = [];
  const base = {
  ...
  if (update.percentComplete !== null) {
    lines.push({ ...base, fieldName: "percent_complete", newValue: String(update.percentComplete) });
  }
  if (update.physicalPercentComplete !== null) {
    lines.push({
      ...base,
      fieldName: "physical_percent_complete",
      newValue: String(update.physicalPercentComplete)
    });
  }
  if (update.actualStart !== null) {
    lines.push({ ...base, fieldName: "actual_start", newValue: update.actualStart });
  }
  if (update.actualFinish !== null) {
    lines.push({ ...base, fieldName: "actual_finish", newValue: update.actualFinish });
  }
  return lines;
}
