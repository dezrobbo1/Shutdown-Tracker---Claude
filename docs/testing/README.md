...
- Project export artifact handoff tests use synthetic export-preview rows and temporary paths only. API tests verify approved-only worker handoff, grouping eligible leaf-task lines, Microsoft Project task UID/ID propagation, generated metadata recording through the existing lifecycle path, the default disconnected client, and the local-profile controller route. Worker tests verify the export artifact endpoint and response mapping without requiring real Project files.
...
- The shared TypeScript API client has unit tests for source-file upload multipart requests, import review paths, task lineage query parameters, export batch lifecycle and artifact handoff JSON requests, typed error handling, and the exposed review surface manifest.
