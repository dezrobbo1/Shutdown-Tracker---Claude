...
- Export-candidate API tests use fake repositories to verify request validation, proposed-value normalization, approval-neutral orchestration, separately appended candidate-bound approval events, project/snapshot isolation, and rejection of caller-authored captured values, fingerprints, or approval state. They do not prove PostgreSQL capture, fingerprint, foreign-key, trigger, or lock behavior.
- Export preview and batch lifecycle tests use fake repositories to verify candidate-ID-only preview requests, exact candidate/approval comparison, whole-batch freshness failure, draft-only approve/reject transitions, generated artifact metadata recording, manual Project reopen/verification metadata transitions, audit events, and no Project write-back.
- The PostgreSQL migration suite separately proves database-derived baseline/task identity, database-computed fingerprints, candidate/approval/line relationships, immutability, legacy preservation, policy enforcement, transaction rollback, and deterministic lock behavior. Neither test layer is presented as evidence for the other.
