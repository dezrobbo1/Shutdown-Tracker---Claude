...
- Export preview tests use fake repositories and synthetic task/source IDs only. They verify existing approval/export enum values, allowed progress/actual fields, draft preview creation, leaf-task eligibility, summary-task exclusion, unapproved-source exclusion, audit event requests for preview creation, and no-artifact/no-write-back response copy without requiring PostgreSQL or real Project files.
