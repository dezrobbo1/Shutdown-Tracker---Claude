...
- Task 18, add API client package: partially addressed by a shared TypeScript client for source-file upload orchestration, current import review, task lineage review, export preview, and export batch lifecycle endpoints. It does not implement auth, live data fetching, offline sync, worker calls, export artifact generation, or Project write-back.
