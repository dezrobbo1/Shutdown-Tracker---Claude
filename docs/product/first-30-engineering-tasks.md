...
- Import batch persistence: partially addressed by local-profile API services using the existing `import_batches` table and `import_batch_status` enum values. No public import-batch endpoint, parse summary persistence, project snapshot, or imported task persistence exists yet.
- Worker parse handoff: partially addressed by shared Java request/response records, an API request builder with a disconnected job client, and a worker service that maps local source file URIs to MPXJ summary output. No queue, background job, public endpoint, parse summary persistence, project snapshot, imported task persistence, or API-side parsing exists yet.
