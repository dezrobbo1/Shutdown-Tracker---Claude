...
- Task 11, add object-storage abstraction: partially addressed for source files and generated export artifacts by internal API storage abstractions with local filesystem implementations. This is not production object storage.
...
- Task 30, run manual Project reopen test: partially addressed by API endpoints that record manual Microsoft Project reopen and verification metadata against generated export batches using existing export batch status values. This still does not automate Microsoft Project, verify artifact contents automatically, attach manual test evidence, or write back to Microsoft Project.
...
This repository now has a minimal Maven/Spring Boot backend scaffold with PostgreSQL JDBC, Flyway runtime wiring, a worker-only MPXJ import summary spike, a worker-only synthetic MSPDI/XML export artifact spike, an API-only source-file validation placeholder, internal source-file and export-artifact storage abstractions, local-profile source-file upload orchestration, local-profile services for review project bootstrap, source-file metadata persistence, import batch persistence, worker parse-summary handoff, import summary persistence, project snapshot/imported entity persistence, import review API endpoints, task lineage review persistence, export preview model endpoints, export batch approval/generated/reopen/verification metadata orchestration, worker-backed export artifact handoff, audit event writes for upload/review/export mutations, a shared TypeScript API client for upload and import/export review surfaces, shared Java parse/export handoff contracts, structural expected-output checks for the approved synthetic MSPDI fixture and synthetic MSPDI/XML export artifact generation, React/Vite console/mobile PWA shells, opt-in console live reads for import/export review data, and a documented queue/background-job strategy for future worker handoffs. It still does not add seed data, real Project files, Project write-back, queue integration, background consumers, parser execution in the API, automatic lineage matching, live execution state, frontend mutation workflows, mobile offline queues, automated Project verification workflow, public audit query APIs, or production object storage/database setup.
