...
- Source/import/export review smoke script: addressed as a guarded local PowerShell script. Default mode checks health, version, and validation-only source-file handling; write steps require explicit `-AllowWrites` and synthetic local/review inputs.
...
- Export preview model: partially addressed by local-profile endpoints and JDBC services that create approval-neutral immutable candidates, append exact candidate-bound approval events, and create sealed `draft_preview` export batches from candidate IDs only. Export batch approval orchestration is partially addressed by local-profile approve/reject endpoints, manual Microsoft Project reopen/verification metadata endpoints, and a worker-backed artifact generation handoff that alone records generated metadata using existing export batch status values. No frontend/UI, live task update source tables, automated Project verification workflow, or Project write-back exists yet.
- Audit event writes for first review/export mutations: partially addressed by local-profile API wiring that records audit rows for import snapshot accept/reject decisions, task lineage link create/accept/reject decisions, authoritative candidate creation, candidate approval events, export preview creation, export batch approval/rejection, and generated artifact metadata recording using the existing `audit_events` table. No public audit query API, task execution audit events, permission audit events, or offline sync audit events exist yet.
- Task 18, add API client package: partially addressed by a shared TypeScript client for source-file upload orchestration, current import review, task lineage review, approval-neutral export-candidate creation, separate candidate approval events, candidate-ID-only export previews, export batch lifecycle endpoints, and the API export-artifact generation handoff endpoint. It does not implement auth, live data fetching, offline sync, direct worker calls, or Project write-back.
