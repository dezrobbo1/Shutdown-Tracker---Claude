...
- Export preview model: partially addressed by local-profile endpoints and JDBC services that create `draft_preview` export batches and preview line eligibility for explicit approved-source candidates. No export approval, MSPDI/XML artifact generation, frontend/UI, live task update source tables, or Project write-back exists yet.
- Audit event writes for first review mutations: partially addressed by local-profile API wiring that records audit rows for import snapshot accept/reject decisions, task lineage link create/accept/reject decisions, and export preview creation using the existing `audit_events` table. No public audit query API, task execution audit events, permission audit events, export approval audit events, or offline sync audit events exist yet.
- Tasks 14 and 15, scaffold React/Vite console and mobile PWA: addressed as static frontend shells only. The apps have no backend API integration, task execution writes, offline queue, storage, parser execution, export generation, or Project write-back.
...
This repository now has a minimal Maven/Spring Boot backend scaffold with PostgreSQL JDBC, Flyway runtime wiring, a worker-only MPXJ import summary spike, a worker-only synthetic MSPDI/XML export artifact spike, an API-only source-file validation placeholder, an internal source-file storage abstraction, local-profile services for review project bootstrap, source-file metadata persistence, import batch persistence, import summary persistence, project snapshot/imported entity persistence, import review API endpoints, task lineage review persistence, export preview model endpoints, audit event writes for the first review mutations, a shared parse summary handoff contract, structural expected-output checks for the approved synthetic MSPDI fixture, and static React/Vite console/mobile PWA shells. It still does not add frontend API integration, seed data, real Project files, Project write-back, queue integration, automatic lineage matching, live execution state, mobile offline queues, export approval API orchestration, persisted export artifact metadata, public audit query APIs, or a production database setup.
