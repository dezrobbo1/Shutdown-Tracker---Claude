...
- Review project bootstrap and source-file metadata persistence: partially addressed by local-profile API services using the existing `projects` and `source_files` tables. No public upload/storage endpoint, import batch, parser handoff, snapshot, or task persistence exists yet.
- Task 19, implement Project upload endpoint: partially addressed by a validation-only source-file upload placeholder with hardened multipart error responses. It validates multipart metadata and extension only; no storage, MPXJ parsing, import batch, persistence, worker integration, or write-back exists.
...
This repository now has a minimal Maven/Spring Boot backend scaffold with PostgreSQL JDBC, Flyway runtime wiring, a worker-only MPXJ import summary spike, an API-only source-file validation placeholder, an internal source-file storage abstraction, and local-profile services for review project bootstrap/source-file metadata persistence. It still does not add React/Vite, frontend code, domain behavior, seed data, real Project files, Project write-back, import batch persistence, parser handoff, or a production database setup.
