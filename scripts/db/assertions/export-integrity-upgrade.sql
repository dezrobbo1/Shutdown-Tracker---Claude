...
SELECT validation.assert_true(
  ...
  'V006 business rows or values changed during the populated upgrade'
);

CREATE TEMP TABLE actual_policy1_fingerprints (
  relation_name TEXT PRIMARY KEY,
  row_count BIGINT NOT NULL,
  business_hash TEXT NOT NULL
);

INSERT INTO actual_policy1_fingerprints
...
SELECT validation.assert_true(
  NOT EXISTS (
    SELECT 1
    FROM validation.pre_v008_policy1_fingerprints expected
    FULL JOIN actual_policy1_fingerprints actual USING (relation_name)
    WHERE expected.row_count IS DISTINCT FROM actual.row_count
       OR expected.business_hash IS DISTINCT FROM actual.business_hash
       OR expected.relation_name IS NULL
       OR actual.relation_name IS NULL
  ),
  'V007 policy-1 rows or values changed during the V008 upgrade'
);
...
SELECT validation.assert_true(
  NOT EXISTS (
    ...
    FROM export_batches
    WHERE integrity_policy_version IS NULL
      AND line_set_sealed IS NOT NULL
  ),
...
SELECT validation.assert_true(
  (SELECT count(*) = 2
   ...
   WHERE id IN (
     ...
     AND approval_event_order IS NOT NULL
     AND authoritative_export_candidate_id IS NULL
     AND candidate_binding_policy_version IS NULL),
  'V007 approvals must retain event order while remaining unbound policy-1 history'
);
...
SELECT validation.assert_true(
  (SELECT count(*) = 2
   ...
   WHERE integrity_policy_version = 1
     AND authoritative_export_candidate_id IS NULL
     AND captured_source_event_or_payload_hash IS NULL
     AND captured_task_external_uid IS NULL
     AND captured_task_external_id IS NULL
     AND captured_task_name IS NULL),
  'V007 policy-1 lines must not be rewritten as policy-2 candidates'
);
...
SELECT validation.expect_failure(
  ...
  '23514',
  'pre-policy-2 export history is read-only'
);
...
SELECT validation.expect_failure(
  'policy-1 draft progression',
  $$UPDATE export_batches
    SET status = 'awaiting_approval'
    WHERE id = '10000000-0000-0000-0000-000000000611'$$,
  '23514',
  'pre-policy-2 export history is read-only'
);

SELECT validation.expect_failure(
  'policy-1 approved progression',
  $$UPDATE export_batches
    SET status = 'generated'
...
SELECT validation.expect_failure(
  'policy-1 line delete',
  $$DELETE FROM export_batch_lines
    WHERE id = '10000000-0000-0000-0000-000000000621'$$,
  '23514',
  'append-only'
);
...
SELECT validation.assert_true(
  (SELECT integrity_policy_version = 2 AND line_set_sealed = false
   FROM export_batches
   WHERE id = '10000000-0000-0000-0000-000000000699'),
  'New export batches must default to an unsealed policy-2 line set'
);

SELECT validation.expect_failure(
  'new policy-1 batch',
  $$INSERT INTO export_batches (
      id, project_id, project_snapshot_id, status, integrity_policy_version, line_set_sealed
    ) VALUES (
...
SELECT validation.expect_failure(
  ...
  '23514',
  'authoritative policy-2 candidate binding'
);

\echo 'Populated V006 and V007 policy-1 upgrade preservation passed.'
